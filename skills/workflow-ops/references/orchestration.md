...
| 波次 / 前置依赖 | **依赖模型 + Provider**：manifest 保存 direct edge；卡正文保留 displayKey + 完成口径 | planning 调 `workflow-dependencies` 生成 DAG，upload 通过 Provider 写入 | `/search?scope=body` 搜引用初筛；**权威判定 = 逐张 GET 前置卡看状态**，关系读回按 Provider（见第三节） |
...
和 inferenceMethod，transitive edge 只用于链路分析，不重复写入 API。方向固定为
