...
1. **建单前查重**——需求、bug、任务都适用。关键词取现象/成果的名词，用户给了单号就按单号精确查。命中疑似同一对象 → 先把 `displayKey` + 标题报给用户，**不默默建第二张**；确认是同一个就改为在旧单上补评论/附件。
...
## 二、`/search` 当前真实能力（已按合同核实，operationId `searchProject`）
