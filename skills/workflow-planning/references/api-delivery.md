...
3. 穷尽读取当前项目 Room 与 Requirement 列表（cursor 走到 `nextCursor` 为空），比对精确标题、高相似标题和描述里的蓝图标记。**`/search` 只对标题做 ILIKE、不做全文检索**，蓝图标记写在 description 里，搜不到——按标记查重必须走列表分页逐条比对，不能用 search 代替。逐个 GET 核对 Room 归属、内容和 `updatedAt`；疑似重复由用户决定复用、补写、更新还是另建，Agent 不自行覆盖。
...
- 网络错误、超时或 5xx 后，先用已知 UUID。没有 UUID 时：先用**精确标题**走 `/search`（它只匹配标题，能命中就够了），未命中再走 Requirement/Room **列表分页**逐条比对描述里的蓝图标记与 Room 归属——蓝图标记在 description 里，`/search` 检索不到，不能因为"搜不到"就判定未落库。只有两条路径都确认不存在才重发。
