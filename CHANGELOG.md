...
- **`workflow-update` 改为「先判安装形态，再查版本」。** 分流升为第 1 节；宿主托管（第 2 节）明确**不读 `version.json`**，改走宿主自己的机制（`claude plugin marketplace update` + `claude plugin update`，并提醒更新后需重启会话），版本真值指向 marketplace 公开仓的 `plugin.json`；手动安装（第 3 节）保留原有的 `cb` 绕缓存、语义化逐段比较与「线上 < 本地 绝不更新」降级红线。安全边界补一条：宿主托管形态不下载任何文件。
