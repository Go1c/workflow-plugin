...
Agent 把单读**全**（正文 + 评论 + 附件 + 验收项，缺一路不算读过）、逐张核对前置单状态，然后**先梳理需求**——把歧义和需要你拍板的决策点连同建议一并摆出来，讨论完才现查 transitions 流转到进行中开工。干活阶段按确认过的拆分**尽可能并行多个子 Agent**（互斥所有权切分、共享热点不并行、子 Agent 不碰凭证与回写）。做完后按固定顺序回写：**这次不做的 TODO 经确认补需求单** → 证据附件 → **统一结构的证据评论**（改动清单、**提交单号（Git commit / PR 或 SVN revision）**、按验收项逐条对照、实际跑过的命令与输出、决策记录、Known gaps 与遗留补单、边界声明）→ 流转到待验收（工作流没有验收态才流转已完成）。**做完不回写等于没做完。**
