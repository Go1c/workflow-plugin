...
插件包含 7 个技能、6 个斜杠命令、14 条硬闸门（G1–G7 落单闸门与 Q1–Q7 QA 闸门），72 项自动化测试以线上 OpenAPI 合同为真值、每周校验一次合同漂移。遵循 [Agent Plugins 1.0.0](https://agent-plugins.org/) 规范，同时兼容 Claude Code marketplace，MIT 许可。
...
Agent 把单读**全**（正文 + 评论 + 附件 + 验收项，缺一路不算读过）、逐张核对前置单状态，然后**先梳理需求**——把歧义和需要你拍板的决策点连同建议一并摆出来，讨论完才现查 transitions 流转到进行中开工。干活阶段按确认过的拆分**尽可能并行多个子 Agent**（互斥所有权切分、共享热点不并行、子 Agent 不碰凭证与回写）。做完后按固定顺序回写：证据附件 → **统一结构的证据评论**（改动清单、按验收项逐条对照、实际跑过的命令与输出、决策记录、Known gaps、边界声明）→ 流转到待验收。**做完不回写等于没做完。**
